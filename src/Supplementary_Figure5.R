...
CL_SUSA_plot <- function(CCLE_ann) {
  CCLE_ann$name <- gsub("_.*", "", CCLE_ann$sampleID)
  CCLE_ann$tissue_name <-  gsub("_", " ", CCLE_ann$tissue)
  Cl_only_testis <- ggplot2::ggplot(CCLE_ann, ggplot2::aes(UMAP_1, UMAP_2)) + 
    ggplot2::geom_point(size=0.5, alpha=0.3, ggplot2::aes(color = tissue)) +
    ggplot2::geom_point(data=dplyr::filter(CCLE_ann, sampleID %in% c("SUSA_TESTIS")),
               ggplot2::aes(UMAP_1, UMAP_2, fill=tissue), pch=21, color='black', size = 3) +  
    ggrepel::geom_text_repel(data=dplyr::filter(CCLE_ann, sampleID %in% c("SUSA_TESTIS")),
...
CL_SUSA_highlight <- function(CCLE_ann) {
  ...
  Cl_only_testis_highlight <- ggplot2::ggplot(dplyr::filter(CCLE_ann, UMAP_1 < 32.5 & UMAP_1 > 29 & UMAP_2 < 26.5 & UMAP_2 > 25),
    ...
    ggplot2::sguides(fill=FALSE, color=ggplot2::guide_legend(title="tissue", ncol=2))
...
Celligner_alignment_testis <- function(alignment) {
  alignment$name <- gsub("_.*", "", alignment$sampleID)
  ...
  alignment_testis <- ggplot2::ggplot(alignment, ggplot2::aes(UMAP_1, UMAP_2, shape = type)) + 
    ...
    ggplot2::geom_point(data = dplyr::filter(alignment, tissue %in% c('germ_cell') & type=='tumor'),
               size=1, alpha=0.5, ggplot2::aes(color = tissue))  +
    ggplot2::geom_point(data=dplyr::filter(alignment, sampleID %in% c("SUSA_TESTIS")),
                        ggplot2::aes(UMAP_1, UMAP_2, fill=tissue), color='black', size = 3, show.legend = F) + 
    ggplot2::scale_shape_manual(values=c(`CL`=21, `tumor`=16)) +
    ggrepel::geom_text_repel(data=dplyr::filter(alignment, sampleID %in% c("SUSA_TESTIS")),
...
Celligner_alignment_testis_highlight <- function(alignment) {
  ...
  alignment_testis_subtype <- ggplot2::ggplot(dplyr::filter(alignment, tissue == 'germ_cell' & UMAP_1 <7),
                                              ggplot2::aes(UMAP_1, UMAP_2, fill=subtype)) + 
    ggplot2::geom_point(alpha=0.7, pch=21, color='white', size=1.5) +
    ggplot2::geom_point(data=dplyr::filter(alignment, sampleID %in% c("SUSA_TESTIS")), 
                        ggplot2::aes(UMAP_1, UMAP_2, fill=tissue), color='black', size = 3, show.legend = F, pch=21) + 
    ggrepel::geom_text_repel(data=dplyr::filter(alignment, sampleID %in% c("SUSA_TESTIS")),
                             ggplot2::aes(UMAP_1, UMAP_2, label=name), color='black', size=3) +
    ggplot2::xlab("UMAP 1") + 
    ggplot2::ylab("UMAP 2") + 
...
plot_misanntations_melanoma <- function(alignment) {
  melanoma_cluster <- dplyr::filter(alignment, tissue=='skin')$cluster %>%
    table() %>%
    as.data.frame() %>%
  ...
  melanoma_cluster_data$name <- gsub("_.*", "", melanoma_cluster_data$sampleID)
  melanoma_cluster_data$tissue_name <- gsub("_", " ", melanoma_cluster_data$tissue)
  ...
  misannotation_plot <- ggplot2::ggplot(melanoma_cluster_data, ggplot2::aes(UMAP_1, UMAP_2, size=type, color=type)) +
    ...
    ggplot2::geom_point(data = dplyr::filter(melanoma_cluster_data, type=='CL' & tissue != 'skin'),
                        ggplot2::aes(fill=tissue_name), color='black', alpha=0.7, pch=21) +
    ggrepel::geom_label_repel(data = dplyr::filter(melanoma_cluster_data, type=='CL' & tissue != 'skin'),
    ...
    ggplot2::theme(legend.position = 'bottom',
          ...
          axis.text =ggplot2::element_text(size = 6)) +
    ggplot2::guides(size=FALSE, fill=ggplot2::guide_legend(nrow=2,byrow=TRUE,title='tissue')) 
